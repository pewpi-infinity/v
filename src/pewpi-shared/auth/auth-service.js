...
import { createModel } from '../models/client-model.js';
...
class AuthService {
  ...
  /**
   * Restore session from storage (for initialization)
   */
  async restoreSession() {
    this._loadCurrentUser();
    if (this.currentUser) {
      console.log('[AuthService] Session restored for:', this.currentUser.email);
      this._emitEvent('pewpi.login.changed', { user: this.currentUser, action: 'restored' });
    }
    return this.currentUser;
  }
